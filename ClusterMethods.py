...
from sklearn.cluster import MiniBatchKMeans, AgglomerativeClustering, DBSCAN, SpectralClustering, AffinityPropagation
...
def AffinityClust(list_c_args):
    ...
    #
    # Requirements:
    # package sklearn.cluster.AffinityPropagation
    #
    # Inputs:
    # list_c_args
    # Type: list
    # Desc: the list of parameters for the clustering object
    # list_c_args[0] -> typpe: <float>, damping effect from 0.5 to 1.0, default = 0.5
    # list_c_args[1] -> type: <int>, convergance interator, default = 15
    # list_c_args[2] -> type: <string>, distance metric, default = 'euclidean'
    #    
    # Important Info:
    # None
    #
    ...
    ###############################################################################################
    ###############################################################################################
    if len(list_c_args) == 0:
        return AffinityPropagation()
    else:
        return AffinityPropagation(damping = list_c_args[0], convergence_iter = list_c_args[1], affinity = list_c_args[2])
