...
def SqlGenConnection(m_user, m_host, m_pswd, m_db_name):
    ...
    try:
        ...
        sql_conn = 'Could not connect to the server.  Check your arguements'
...
def SqlGetTableColumns(m_sql_conn, m_table_name):
    ...
    str_query_col = SqlGenSelectStatement(m_str_select = 'COLUMN_NAME', 
                                                                       ...
                                                                       m_str_where = "TABLE_NAME = N'" + m_table_name + "'",
...
def SqlTestTableExists(m_sql_connection, m_table_name):
    ...
    # variables
    str_query = SqlGenSelectStatement('if exists (', '*', 'INFORMATION_SCHEMA.TABLES', "TABLE_NAME = N'" + m_table_name, "') select 1 else select 0")
    # str_query += r"if exists (select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME = N'" + m_table_name + r"') select 1 else select 0;"
...
def SqlCreateTable(m_sql_connection, m_table_name, m_list_columns, m_bool_wide_table = True):
    ...
    # matrix to account for data that is not in a column
    #
    # Return:
    # object
    # Type: list
...
def SqlUpdate(m_sql_conn, m_table_name, m_list_columns, m_list_col_data_type, m_list_values, m_list_where):
    ...
    # variable
    # Type: boolean
    # Desc: return true of the statement is executed with no errors, return false if executed with errors
    ###############################################################################################
    ###############################################################################################
...
def SqlCheckColumns(list_columns = [], list_check = []):
    ###############################################################################################
    ###############################################################################################
    #
    # this method checks the contents of two lists to see if they are the same, one list represents the columns of a table
    # and the second list represents the names to check
    #
    # Requirements:
    # none
    #
    ...
    # variable
    # Type: boolean
    # Desc: True if all the columns in list_check are in list_columns, else return false
    ###############################################################################################
    ###############################################################################################
    ...
    for str_check in list_check:
        if str_check not in list_columns:
            bool_return = False
            break
